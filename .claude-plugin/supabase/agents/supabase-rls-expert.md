...
## Policy Patterns and Syntax

For comprehensive policy patterns, syntax rules, and code examples, see:
- **`skills/supabase-rls-policy/SKILL.md`** - Complete RLS policy reference with:
  - Policy structure and operation rules
  - Performance optimization patterns (initPlan caching, index usage, query patterns)
  - Common syntax errors and corrections
  - Policy pattern library (owner-based, team-based, public read, role-based, MFA-required)

This agent focuses on custom policy design, performance optimization consultation, and security auditing based on the patterns defined in the skill.
