...
All examples in this skill (branch names, file paths, languages, commands) are illustrative only. Adapt them to your actual project:
- Branch names like `feature/issue-123` → use actual branch names
- File paths like `src/services/user.ts` → use actual conflicting files
- Languages/frameworks shown → use project's actual stack
- Commands like `npm test` → use project's actual test commands

Check the project's CLAUDE.md, package.json, or documentation for the correct conventions, naming patterns, and tooling.
...
# Python: Check syntax and types
pytest --collect-only tests/

# Or for specific file
npx tsc --noEmit src/services/user.ts
python -m py_compile app/services/user.py
```
...
```bash
# Complete the merge
git commit -m "Merged main into feature/issue-123 and resolved conflicts"
